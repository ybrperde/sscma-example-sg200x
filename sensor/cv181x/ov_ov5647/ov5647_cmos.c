...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		/* Keep u8ImgMode from cmos_set_image_mode(); do not force 1080p. */
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	printf("ov5647 set_image_mode req %dx%d fps=%.1f (cur mode=%d init=%d)\n",
	       pstSensorImageMode->u16Width, pstSensorImageMode->u16Height,
	       pstSensorImageMode->f32Fps, pstSnsState->u8ImgMode, pstSnsState->bInit);
	...
	/* PQ bin / AE / thermal may ask for 1080p@30 after 5MP is already running. Stay on 5MP. */
	if (pstSnsState->bInit == CVI_TRUE &&
	    pstSnsState->u8ImgMode == OV5647_MODE_2592X1944P15 &&
	    u8SensorImageMode != OV5647_MODE_2592X1944P15) {
		printf("ov5647: ignore switch from 5MP to mode %d (%dx%d)\n",
		       u8SensorImageMode, pstSensorImageMode->u16Width, pstSensorImageMode->u16Height);
		return CVI_FAILURE;
	}
