...
static void ov5647_linear_5m_15fps_init(VI_PIPE ViPipe);
...
void ov5647_init(VI_PIPE ViPipe)
{
	...
	switch (g_pastOv5647[ViPipe]->u8ImgMode) {
	case OV5647_MODE_2592X1944P15:
		ov5647_linear_5m_15fps_init(ViPipe);
		break;
	default:
...
/* 2592x1944 @~15fps 10bit. Same CVI MIPI/PLL sequence as 1080p (proven on this board);
 * only crop / HTS / VTS change, so CSI lane rate stays at the working 1080p clock. */
static void ov5647_linear_5m_15fps_init(VI_PIPE ViPipe)
{
	ov5647_write_register(ViPipe, 0x0100, 0x00);
	ov5647_write_register(ViPipe, 0x0103, 0x01);
	ov5647_write_register(ViPipe, 0x3035, 0x11);
	ov5647_write_register(ViPipe, 0x3036, 0x64);  /* same PLL as 1080p → same MIPI rate */
	ov5647_write_register(ViPipe, 0x303c, 0x11);
	ov5647_write_register(ViPipe, 0x3821, 0x00);
	ov5647_write_register(ViPipe, 0x3820, 0x00);
	ov5647_write_register(ViPipe, 0x370c, 0x0f);
	...
	ov5647_write_register(ViPipe, 0x3c01, 0x80);
	ov5647_write_register(ViPipe, 0x3b07, 0x0c);
	/* HTS = 2844, VTS = 1968 → ~14.3 fps at the 1080p pixel clock */
	...
	ov5647_write_register(ViPipe, 0x4050, 0x6e);
	ov5647_write_register(ViPipe, 0x4051, 0x8f);
	/* Same post-stream MIPI/pad enable as 1080p (required for CVI RX lock) */
	ov5647_write_register(ViPipe, 0x0100, 0x01);
	ov5647_write_register(ViPipe, 0x3000, 0x00);
	ov5647_write_register(ViPipe, 0x3001, 0x00);
	ov5647_write_register(ViPipe, 0x3002, 0x00);
	...
	printf("ViPipe:%d,===OV5647 5MP 15fps 10bit LINE Init OK! "
	       "out=%02x%02x x %02x%02x stream=%02x mipi=%02x pll=%02x/%02x\n",
	       ViPipe,
	       ov5647_read_register(ViPipe, 0x3808), ov5647_read_register(ViPipe, 0x3809),
	       ov5647_read_register(ViPipe, 0x380a), ov5647_read_register(ViPipe, 0x380b),
	       ov5647_read_register(ViPipe, 0x0100), ov5647_read_register(ViPipe, 0x3018),
	       ov5647_read_register(ViPipe, 0x3035), ov5647_read_register(ViPipe, 0x3036));
}
